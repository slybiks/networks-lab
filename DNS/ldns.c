...
int main() {
    ...
    int len, n;
    char filename[100],abspath[100],find[100],fullpath[100],display[100],contents[500];
    ...
    close(s);
     ...
     if( fullpath[0] != '\0' ){
	    ...
	    fgets(contents,sizeof(contents),p);
	    close(p);

	    sendto(sockfd,contents,strlen(contents), MSG_CONFIRM, (const struct sockaddr *) &cliaddr,len);
